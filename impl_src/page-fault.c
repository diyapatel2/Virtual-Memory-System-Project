...
pfn_t pagefault_handler(vpn_t request_vpn, int write)
{
	...
	{
		if(IS_SET(rlt[victim_pfn].pcb->pagetable[rlt[victim_pfn].vpn].flags, 1))
		{
			page_to_disk(victim_pfn, rlt[victim_pfn].vpn, rlt[victim_pfn].pcb->pid);
		
		}
			CLEAR_BIT(rlt[victim_pfn].pcb->pagetable[rlt[victim_pfn].vpn].flags, 0);/*valid*/
	...
	SET_BIT(current_pagetable[request_vpn].flags, 2);/*USED*/
	if(write) {
		SET_BIT(current_pagetable[request_vpn].flags, 1); /*dirty*/
	}
